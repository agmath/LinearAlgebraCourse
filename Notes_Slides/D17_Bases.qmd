...
<img src="images/BasisMap.png" width="300">
...
**Testing for Linear Independence and Spanning:** Consider a collection of vectors $V = \left\{\vec{v_1},~\vec{v_2}~,\cdots,~\vec{v_p}\right\}$ from $\mathbb{R}^m$, and the matrix $A = \begin{bmatrix} \vec{v_1} & \vec{v_2} & \cdots & \vec{v_p}\end{bmatrix}$ whose columns are the vectors from $V$.

+ The vectors in $V$ are *linearly independent* if the REF form of $A$ has a pivot in every column.
+ The vectors in $V$ *span* $\mathbb{R}^m$ if the REF form of $A$ has a pivot in every row.
...
*Solution.* This collection cannot be a basis for $\mathbb{R}^4$ since it contains only three vectors. 
...
*Solution.* We'll use the vectors as columns in a matrix $A$ and then determine whether that matrix has a pivot in every row and every column.
...
Since there is a *pivot* in every row and every column of the matrix $A$, then the collection is both *linearly independent* and *spans* $\mathbb{R}^3$, and so the collection forms a *basis*.
...
## Transformations Between Bases $\left(\mathscr{B} \to \mathscr{B}_S\right)$

. . . 

It is possible to construct a linear transformation to convert between a non-standard basis $\mathscr{B}$ and our standard basis $\mathscr{B}_S$

. . . 

**Strategy:** Given a nonstandard basis $\mathscr{B} = \left\{v_1,~v_2,~\cdots,~v_n\right\}$ and a vector $\left[\vec{x}\right]_{\mathscr{B}}$ written in that nonstandard basis, we can find the coordinates of $\left[\vec{x}\right]_{\mathscr{B}}$ in the standard basis by evaluating the matrix-vector product

$$\begin{bmatrix} \vec{v_1} & \vec{v_2} & \cdots & \vec{v_n}\end{bmatrix}\left[\vec{x}\right]_{\mathscr{B}}$$

. . . 

**Transformation Viewpoint:** The matrix transformation $T:~\mathbb{R}^n\to \mathbb{R}^n$ defined by 

$$T\left(\left[\vec{x}\right]_{\mathscr{B}}\right) = \begin{bmatrix} \vec{v_1} & \vec{v_2} & \cdots & \vec{v_n}\end{bmatrix}\left[\vec{x}\right]_{\mathscr{B}}$$

converts between the nonstandard basis $\mathscr{B}$ and the standard basis $\mathscr{B}_S$.

## Completed Example: Converting from $\mathscr{B}$ to $\mathscr{B}_S$

. . . 

<div style="font-size:18pt">

**Example:** Consider $\mathscr{B} = \left\{\begin{bmatrix} 2\\ -5\\ 0\end{bmatrix}, \begin{bmatrix} 0\\ 11\\ -12\end{bmatrix}, \begin{bmatrix} 1\\ 1\\ 1\end{bmatrix}\right\}$. Verify that $\mathscr{B}$ is a basis for $\mathbb{R}^3$ and then convert the vector $\left[\vec{x}\right]_{\mathscr{B}} = \begin{bmatrix} -3\\ 7\\ 12\end{bmatrix}$ into its corresponding standard basis coordinates.
...
So $\mathscr{B}$ does form a basis for $\mathbb{R}^3$ because the corresponding matrix $A$ has a pivot in every row and every column.
...
So the vector $\left[\vec{x}\right]_{\mathscr{B}} = \begin{bmatrix} -3\\ 7\\ 12\end{bmatrix}$ has coordinates $\begin{bmatrix} 6\\ 104\\ -72\end{bmatrix}$ in the standard basis.
...
## Transformations Between Bases $\left(\mathscr{B}_S \to \mathscr{B}\right)$

. . . 

<div style="font-size:18pt">

Similarly, it is possible to construct a linear transformation to convert between the standard basis $\mathscr{B}_S$ and any nonstandard basis $\mathscr{B}$
...
**Strategy:** Given a nonstandard basis $\mathscr{B} = \left\{v_1,~v_2,~\cdots,~v_n\right\}$ and a vector $\vec{x}$ written in the standard basis, we can find the coordinates of $\vec{x}$ in the nonstandard basis by evaluating the matrix-vector product

$$\left(\begin{bmatrix} \vec{v_1} & \vec{v_2} & \cdots & \vec{v_n}\end{bmatrix}\right)^{-1}\vec{x}$$
...
Converts between the nonstandard basis $\mathscr{B}$ and the standard basis $\mathscr{B}_S$, so the matrix transformation $T^{-1}: \mathbb{R}^n\to \mathbb{R}^n$ defined by

$$T^{-1}\left(\vec{x}\right) = \left(\begin{bmatrix} \vec{v_1} & \vec{v_2} & \cdots & \vec{v_n}\end{bmatrix}\right)^{-1}\vec{x}$$

converts from the standard basis $\mathscr{B}_S$ to the nonstandard basis $\mathscr{B}$.

</div>

## Completed Example: Converting from $\mathscr{B}_S$ to $\mathscr{B}$

. . . 

<div style="font-size:18pt">

**Example:** Consider $\mathscr{B} = \left\{\begin{bmatrix} 2\\ -5\\ 0\end{bmatrix}, \begin{bmatrix} 0\\ 11\\ -12\end{bmatrix}, \begin{bmatrix} 1\\ 1\\ 1\end{bmatrix}\right\}$. Convert the vector $\vec{x} = \begin{bmatrix} 6\\ 104\\ -72\end{bmatrix}$ into its corresponding $\mathscr{B}$-coordinates.
...
So the vector $\vec{x} = \begin{bmatrix} 6\\ 104\\ -72\end{bmatrix}$ has $\mathscr{B}$-coordinates $\begin{bmatrix} -3\\ 7\\ 12\end{bmatrix}_{\mathscr{B}}$.
...
Navigate to [our MAT350 Exit Ticket Form](https://docs.google.com/forms/d/e/1FAIpQLSc8TmycI06yujoFNBwAvvbBZbCruUF6dPZZkC49aQ20ZLjDAA/viewform?usp=header), answer the questions, and complete the *task* below.

<br/>

*Note.* Today's discussion is listed as `15. Bases and Coordinate Systems`

<br/>

**Task:** Given the vector $\left[\vec{x}\right]_{\mathscr{B}} = \begin{bmatrix} 2\\ -4\end{bmatrix}$ written in the nonstandard basis $\mathscr{B} = \left\{\begin{bmatrix} 7\\ 11\end{bmatrix},~\begin{bmatrix} 6\\ 7\end{bmatrix}\right\}$, find the coordinates of the vector in the standard basis.

## Summary

+ A *basis* for a space $H$ is a collection of vectors from $H$ which is both *linearly independent* and *spans* $H$.
+ Regarding bases for $\mathbb{R}^n$, a collection of vectors from $\mathbb{R}^n$ forms a basis for $\mathbb{R}^n$ if the matrix $A$, whose columns are formed by those vectors, has a pivot in every row and every column.
+ The *dimension* of a space is the number of vectors in any basis for that space.
+ **Converting Between Bases:** Multiplying a vector by the matrix whose columns are the nonstandard basis vectors converts from nonstandard to standard basis coordinates, and multiplying by its inverse converts from standard to nonstandard coordinates.
