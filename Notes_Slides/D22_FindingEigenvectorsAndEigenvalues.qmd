...
+ A scalar $\lambda$ is an *eigenvalue* of $A$ if $\left(A - \lambda I_n\right)\vec{x} = \vec{0}$ has a non-trivial solution.
...
For example, what if we wanted to find the eigenvalues corresponding to the matrix $A = \left[\begin{array}{rr} 1 & 4\\ 3 & 2\end{array}\right]$? What about finding the bases for the eigenspaces corresponding to the eigenvectors?
...
+ There will be infinitely many *eigenvectors* corresponding to any *eigenvalue*, so we really seek a *basis* for the *eigenspace*!
...
1. Construct the augmented matrix $\left[\begin{array}{r|r} A - \lambda I & \vec{0}\end{array}\right]$
...
2. if $U$ is obtained from $A$ by *scaling* a single row by the scalar $s$, then $\text{det}\left(U\right) = s\cdot\text{det}\left(A\right)$
...
**Comment:** The largest, non-triangular, matrices we'll work with "by hand" in MAT350 are $3\times 3$.
...
*Note.* Today's discussion is listed as `20. Finding Eigenvalues and Eigenvectors`

<br/>

**Task:** Find all eigenvalues of the matrix $A = \begin{bmatrix} -1 & 2\\ 2 & 2\end{bmatrix}$ and construct a corresponding eigenbasis for each of the corresponding eigenspaces.
